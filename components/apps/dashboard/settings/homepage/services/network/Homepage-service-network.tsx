...
const HomepageServiceNetworks: React.FC = () => {

  const [serviceNetworks, setServiceNetworks] = useState<ServiceNetwork[]>([
    {
      id: '1',
  ...
  const [viewServiceNetwork, setViewServiceNetwork] = useState<ServiceNetwork | null>(null);
  const [deleteServiceNetwork, setDeleteServiceNetwork] = useState<ServiceNetwork | null>(null);

  const handleDelete = (service: ServiceNetwork) => {
    setServiceNetworks(prev => prev.filter(cap => cap.id !== service.id));
    setDeleteServiceNetwork(null);
  };
    ...
    <div className="container mx-auto" style={{backgroundColor: themeConfig.backgroundColor}}>
      <div className="bg-white rounded-lg shadow-lg overflow-hidden">
        <div className="border-b border-gray-200 p-6">
          <h1 className="text-2xl font-bold text-gray-900">Service Network List</h1>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {serviceNetworks.map((service, index) => (
                <tr key={service.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex justify-end gap-2">
                      ...
                      <button
                        onClick={() => setDeleteServiceNetwork(service)}
      ...
      {viewServiceNetwork && (
        <ViewModal
          service={viewServiceNetwork}
          onClose={() => setViewServiceNetwork(null)}
        />
      )}
      ...
      {deleteServiceNetwork && (
        <DeleteModal
          service={deleteServiceNetwork}
          onConfirm={() => handleDelete(deleteServiceNetwork)}
          onCancel={() => setDeleteServiceNetwork(null)}
        />
      )}
    </div>
  );
...
export default HomepageServiceNetworks;